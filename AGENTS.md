...
- Authentication config lives in `apps/web/src/auth/` (server and client modules). Auth-related database tables are generated into `packages/db/src/auth-schema.ts` (auto-generated by `pnpm auth:generate`; do not edit manually).
- Database schema, client, and migration runner live in `packages/db/src/*`. Drizzle config lives at `packages/db/drizzle.config.ts`, and SQL migrations remain in the repo root `migrations/` directory.
...
Use pnpm for everything. `pnpm dev` starts the Vite dev server for `apps/web` on port 3000 with hot reload. `pnpm build` builds all packages via turborepo, while `pnpm preview` runs the Nitro server from `apps/web/.output/` to sanity-check SSR. `pnpm check:types` runs typechecking across packages. `pnpm lint` applies the TanStack + React ESLint rules, and `pnpm format` runs Prettier then auto-fixes lint errors.
