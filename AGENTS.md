...
- App packages live in `apps/*` and shared packages live in `packages/*`.
- The main app is `apps/web`.
- Source for the web app lives in `apps/web/src/`. Global routing is defined in `apps/web/src/router.tsx`, with route modules in `apps/web/src/routes` (`__root.tsx` for layout, `index.tsx` for the landing page).
- Shared UI composites belong in `apps/web/src/components`, design primitives in `apps/web/src/ui`. Styling overrides sit in `apps/web/src/styles.css`. Static files stay in `apps/web/public/`, and production artifacts are written to `.output/` (Nitro).
- Background jobs live in `apps/worker` and run a Restate server (`apps/worker/src/restate/app.ts`) on port `9080`.
- Database schema, client, and migration runner live in `packages/db/src/*`. Drizzle config lives at `packages/db/drizzle.config.ts`, and migrations are generated into `packages/db/migrations` (with symlinks from root `migrations/` and `apps/web/migrations`).
- `.env`, `.env.example`, `.envrc` remain in the repo root only.
...
- `pnpm dev` / `pnpm dev:web`: start the Vite dev server for `apps/web` on port `3000` with hot reload.
- `pnpm dev:worker`: start the Restate worker server on port `9080` by default (override with `PORT`), and it runs DB migrations on startup.

- `pnpm build`: build all packages via turborepo.
- `pnpm build:web`: build `apps/web`.
- `pnpm build:worker`: build `apps/worker`.

- `pnpm preview` / `pnpm preview:web`: run the Nitro server from `apps/web/.output/` to sanity-check SSR.
- `pnpm preview:worker`: start the worker (same as `pnpm --filter @proj/worker start`).

- `pnpm test`: run tests across packages.
- `pnpm test:web`: run tests for `apps/web`.
- `pnpm test:worker`: run tests for `apps/worker` (currently a no-op placeholder).

- `pnpm lint`: apply the TanStack + React ESLint rules.
- `pnpm lint:web`: lint `apps/web`.
- `pnpm lint:worker`: lint `apps/worker`.

- `pnpm format`: run Prettier then auto-fixes lint errors.
- `pnpm format:web`: format/lint-fix `apps/web`.
- `pnpm format:worker`: format/lint-fix `apps/worker`.

- `pnpm check:types`: run typechecking across packages.
- `pnpm check:types:web`: typecheck `apps/web`.
- `pnpm check:types:worker`: typecheck `apps/worker`.
...
4. For Restate jobs, any external I/O (e.g. GitHub API calls) must be wrapped in `ctx.run(...)` for observability and retries.
